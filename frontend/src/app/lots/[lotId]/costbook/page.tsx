...
function confidence(n?: number | null) {
  ...
  if (n >= 0.7) return "text-[var(--ch-accent)]";
  return "text-[var(--ch-error-text)]";
}
      ...
      <div className="grid grid-cols-3 gap-4 mb-6">
        {[
          ...
          { label: "Total Actual", value: budget.total_actual, color: "text-[var(--ch-text-primary)]" },
          {
            ...
            color: budget.total_variance > 0 ? "text-[var(--ch-error-text)]" : budget.total_variance < 0 ? "text-[var(--ch-success-text)]" : "text-[var(--ch-text-primary)]",
          },
                ...
                {lines.map((line) => {
                  ...
                  return (
                    <tr key={line.id} className="border-t border-[var(--ch-border)] hover:bg-[var(--ch-surface)] group">
                      ...
                      <td className="px-4 py-2.5 text-[var(--ch-text-primary)]">{line.description}</td>
                      ...
                      <td className="px-4 py-2.5 text-right">
                        {editingCell?.lineId === line.id && editingCell.field === "estimate" ? (
                          <input
                            ...
                            className="w-full text-right bg-[var(--ch-page-bg)] border border-[var(--ch-amber)] rounded px-2 py-0.5 text-[var(--ch-text-primary)] focus:outline-none"
                          />
                        ) : (
                          <span
                            onClick={() => startEdit(line, "estimate")}
                            className="cursor-pointer hover:text-[var(--ch-accent)] transition-colors text-[var(--ch-text-secondary)]"
                          >
                      ...
                      {/* Variance */}
                      <td className={`px-4 py-2.5 text-right font-medium ${isOver ? "text-[var(--ch-error-text)]" : isUnder ? "text-[var(--ch-success-text)]" : "text-[var(--ch-text-muted)]"}`}>
                      ...
                      <td className="px-4 py-2.5 text-center opacity-0 group-hover:opacity-100 transition-opacity">
                        <button
                          ...
                          title="Issue PO"
                          className="text-xs text-[var(--ch-text-muted)] hover:text-[var(--ch-accent)] transition-colors"
                        >
                          PO+
                        </button>
                      </td>
...
const PO_STATUS_COLOR: Record<string, string> = {
  draft: "bg-[var(--ch-page-bg)] text-[var(--ch-text-secondary)]",
  issued: "bg-[var(--ch-upcoming-badge-bg)] text-[var(--ch-upcoming-badge-text)]",
  acknowledged: "bg-[var(--ch-amber-bg)] text-[var(--ch-amber-text)]",
        ...
        <tbody>
          {pos.map((po) => (
            <tr key={po.id} className="border-t border-[var(--ch-border)] hover:bg-[var(--ch-surface)]">
              <td className="px-4 py-3 font-mono text-xs text-[var(--ch-text-secondary)]">{po.po_number}</td>
              <td className="px-4 py-3 text-[var(--ch-text-primary)]">{po.vendor_name || po.vendor_name_adhoc || "—"}</td>
              <td className="px-4 py-3 text-[var(--ch-text-secondary)]">{po.description}</td>
              <td className="px-4 py-3 text-right text-[var(--ch-text-primary)]">{fmt(po.amount)}</td>
    ...
    <div>
      ...
      <div className="mb-6">
        <label className={`inline-flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-medium cursor-pointer transition-all
          ${uploading
            ? "bg-[var(--ch-page-bg)] text-[var(--ch-text-muted)] cursor-not-allowed"
            : "bg-[var(--ch-amber-bg)] text-[var(--ch-accent)] border border-[var(--ch-amber)] hover:bg-[var(--ch-amber-bg)]"
          }`}
        >
          ...
          {invoices.map((inv) => (
            <div key={inv.id} className="rounded-xl border border-[var(--ch-border)] bg-[var(--ch-surface)] p-5">
                ...
                <div>
                  <div className="flex items-center gap-2 mb-1">
                    <span className={`text-xs font-semibold px-2 py-0.5 rounded-full ${
                      ...
                      : "bg-[var(--ch-amber-bg)] text-[var(--ch-accent)]"
                    }`}>
                  ...
                  {approvingId === inv.id ? (
                    <>
                      <select
                        ...
                        className="flex-1 bg-[var(--ch-page-bg)] border border-[var(--ch-border-strong)] rounded-lg px-3 py-1.5 text-sm text-[var(--ch-text-primary)] focus:outline-none focus:border-[var(--ch-accent)]"
                      >
                      ...
                      <button
                        ...
                        className="px-3 py-1.5 rounded-lg bg-[var(--ch-success-bg)] text-[var(--ch-success-text)] text-sm hover:bg-[var(--ch-success-bg)] transition-colors"
                      >
                        Confirm
                      </button>
                      <button
                        onClick={() => setApprovingId(null)}
                        className="px-3 py-1.5 rounded-lg bg-[var(--ch-surface)] text-[var(--ch-text-muted)] text-sm hover:bg-[var(--ch-page-bg)] transition-colors"
                      >
                        Cancel
                      </button>
                    </>
                    ...
                    <>
                      ...
                      <button
                        ...
                        className="px-3 py-1.5 rounded-lg bg-[var(--ch-error-bg)] text-[var(--ch-error-text)] text-sm transition-colors hover:brightness-95"
                      >
                        Reject
                      </button>
                    </>
              ...
              {inv.status === "rejected" && inv.rejection_reason && (
                <p className="text-xs text-[var(--ch-error-text)]/70 mt-2 pt-2 border-t border-[var(--ch-border)]">{inv.rejection_reason}</p>
              )}
            </div>
          ))}
        </div>
...
function IssuePODrawer({
  ...
  return (
    <div className="fixed inset-0 z-50 flex justify-end">
      ...
      <div className="relative w-full max-w-md bg-[var(--ch-surface)] border-l border-[var(--ch-border)] h-full overflow-y-auto p-6 flex flex-col gap-5">
        ...
        <div className="space-y-4">
          <div>
            ...
            <select
              ...
              className="w-full bg-[var(--ch-surface)] border border-[var(--ch-border)] rounded-lg px-3 py-2 text-sm text-[var(--ch-text-primary)] focus:outline-none focus:border-[var(--ch-accent)] mb-2"
            >
            ...
            {!vendorId && (
              <input
                ...
                className="w-full bg-[var(--ch-surface)] border border-[var(--ch-border)] rounded-lg px-3 py-2 text-sm text-[var(--ch-text-primary)] placeholder:text-[var(--ch-text-muted)] focus:outline-none focus:border-[var(--ch-accent)]"
              />
            )}
          </div>
          ...
          <div>
            ...
            <input
              ...
              className="w-full bg-[var(--ch-surface)] border border-[var(--ch-border)] rounded-lg px-3 py-2 text-sm text-[var(--ch-text-primary)] focus:outline-none focus:border-[var(--ch-accent)]"
            />
          </div>
          ...
          <div>
            ...
            <textarea
              ...
              rows={3}
              className="w-full bg-[var(--ch-surface)] border border-[var(--ch-border)] rounded-lg px-3 py-2 text-sm text-[var(--ch-text-primary)] placeholder:text-[var(--ch-text-muted)] focus:outline-none focus:border-[var(--ch-accent)] resize-none"
            />
          </div>
        </div>

        <div className="flex gap-3 mt-auto pt-4 border-t border-[var(--ch-border)]">
          <button
            onClick={submit}
            disabled={saving}
            className="flex-1 py-2.5 rounded-lg bg-[var(--ch-amber-bg)] text-[var(--ch-accent)] border border-[var(--ch-amber)] text-sm font-medium hover:bg-[var(--ch-amber-bg)] transition-colors disabled:opacity-40"
          >
            {saving ? "Issuing…" : "Issue PO"}
          </button>
          <button
            onClick={onClose}
            className="px-4 py-2.5 rounded-lg bg-[var(--ch-surface)] text-[var(--ch-text-muted)] text-sm hover:bg-[var(--ch-page-bg)] transition-colors"
          >
            Cancel
          </button>
        </div>
        ...
        {loading ? (
          ...
          <div className="text-center py-16">
            ...
            <button
              ...
              disabled={creating}
              className="px-5 py-2.5 rounded-lg bg-[var(--ch-amber-bg)] text-[var(--ch-accent)] border border-[var(--ch-amber)] text-sm font-medium hover:bg-[var(--ch-amber-bg)] transition-colors disabled:opacity-40"
            >
              ...
              {TAB_ITEMS.map((tab) => (
                <button
                  ...
                  className={`px-4 py-2 rounded-lg text-sm font-medium transition-all ${
                    activeTab === tab
                      ? "bg-[var(--ch-page-bg)] text-[var(--ch-text-primary)]"
